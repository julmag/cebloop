...
# Supervised training
net.train(i_input[:d_train], i_target[:d_train])

# Autoregressive test
net.test(i_input[d_test:], i_target[d_test:])
data = net.recorder.get()

mse = np.round((np.mean((np.abs(i_target[d_test:,:]) - np.abs(data['readout'][d_test:, :])), axis=0)**2),4)
suptitle = "MSE X: " + str(mse[0]) + "; MSE Y: " + str(mse[1])
# Visualize
plt.figure(figsize=(12, 7))
plt.title("Autoregression")
plt.subplot(211)
plt.plot(i_target[:d_train+d_test,:], label='ground truth')
...
plt.subplot(212)
plt.title(suptitle)
plt.plot(i_target[:d_train+d_test,:] - data['readout'][:, :], label='error')
plt.legend()
plt.show()
...
#video3(data['readout'][:, :])
video4(data['readout'][d_test:, :], i_target[d_test:,:])
print("pogo")
